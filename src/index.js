...
function generateContent(location, level, subject, parentRegion) {
  ...
  const _rsHTML = _rs ? `<h2>${regionDisplay} 인근 ${levelName(_rs.sl)} 정보</h2><p>${pick(SCHOOL_SECTION_POOL, rng)(_rs.dispRegion, _rs.count, levelName(_rs.sl), _rs.picked, subject)}</p>` : '';
...
function getRegionCtx(loc,parent){let sido=null,gugun=null;if(parent===null){sido=loc;}else if(parent.indexOf(' ')>0){const p=parent.split(' ');sido=p[0];gugun=p[1];}else{sido=parent;gugun=loc;}return{sido,gugun};}
...
export default {
  async fetch(request, env) {
    ...
    if (pathname === '/version') {
      return new Response('v80-phase2a-elem-included', { headers: { 'Content-Type': 'text/plain' } });
    }
