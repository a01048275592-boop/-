...
function renderSigunguDetail(sido, sigungu, cat, val) {
  ...
  const intro = [`${kw}는 ${sido} ${sigungu} ${cat?val+' ':''}학생 1:1 맞춤 수업입니다. 동네·학교 특성 반영한 검증 선생님 매칭으로 결과를 만듭니다. 중요한 건 실력보다 '궁합'. 과외안하니는 성향 분석→매칭→무료 체험→교체로 완벽한 궁합을 만듭니다.`];
  const sections = [
    [`${kw} 성적 향상 전략`,`${sigungu} 학생 맞춤 커리큘럼이 효과적. 개념 이해가 핵심. 단순 암기는 응용에서 무너져요. ${kw} 선생님은 반복 확인으로 고득점 만들어요. 시험 3주 전 개념 총정리, 2주 전 기출 분석, 1주 전 예상 문제 풀이, 시험 직전 핵심 요약으로 자신감을 만듭니다. 방문/화상 가능. 지금 ${kw}를 시작하세요.`]
  ];
    return `<!DOCTYPE html><html lang="ko"><head>
    ...
    if (dongMatch) {
      ...
      if (REGIONS[sido]) {
        ...
        if (allDongs.includes(dong)) {
          const html = renderDongCategoryPage(sido, sg, dong, 'lvl', '초등');
          if (html) return new Response(html, {
            headers: {'Content-Type':'text/html;charset=utf-8','Cache-Control':'public,max-age=86400'}
          });
        }
