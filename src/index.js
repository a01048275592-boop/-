...
function commonHead(title, description, canonical, ogImage, viewport) {
  const _img = ogImage || DEFAULT_OG_IMAGE;
  const _vp = viewport || 'width=device-width, initial-scale=1.0';
  return `<meta charset="UTF-8">
  <meta name="naver-site-verification" content="2d31a5395d70375a6b80e71c055be5e739383013" />
  <meta name="viewport" content="${_vp}">
...
function renderHomepage() {
  ...
  const subjectCards=SUBJECTS.map(s=>`<a href="/과목별/${encodeURIComponent(s)}" class="subject-card" style="--accent:${subjectColors[s]}"><span class="subject-icon">${subjectIcons[s]}</span><h3>${s}</h3><p>${s} 과외 정보 보기</p></a>`).join("");
