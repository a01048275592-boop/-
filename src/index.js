...
function renderRegionSubjectAcademy(center, region, subject) {
  ...
  const col = sc[subject] || '#6366f1', bg = sbg[subject] || '#eef2ff';
  const reasonPool = [
    ['🚇','통학이 편한 거리감',region+' '+rtLabel+' 학생들이 도보 또는 짧은 대중교통으로 오갈 수 있는 위치이며 야간 수업 후 귀가 동선까지 고려했습니다.'],
    ['🏘️',region+' 학생 특성 데이터화','같은 생활권에서 수년간 '+subject+'을 지도하며 지역 학생들의 반복 유형과 약점 패턴을 축적해왔습니다.'],
    ['📖','인근 학원가 정보망',region+' 주변 학원가·학부모 커뮤니티와 소통하며 '+subject+' 교재·시험 트렌드를 빠르게 반영합니다.'],
    ['🎯','맞벌이 가정 시간대 배려','맞벌이 가정이 많은 '+region+' 특성을 반영해 귀가 시간대·주말 오전 등 다양한 수업 옵션을 운영합니다.'],
    ['🧩','형제·자매 동반 편의','한 지점에서 남매·자매가 함께 수업받는 경우가 많아 시간 조율과 동반 혜택을 유연하게 제공합니다.'],
    ['🔎','또래 소그룹 편성 가능','희망 시 '+region+' 거주 동일 학년 소그룹 편성이 가능하며 또래 학습 동기 효과를 활용할 수 있습니다.'],
  ];
  const reasons = pickN(reasonPool, 4, rng);
  const subjPlan = {
    '국어':['학년별 권장 독서 리스트 기반 독해 훈련','어휘 앱 연동 매일 누적 테스트','문학·비문학 영역별 오답 분류','서술형 3문장 요약 루틴'],
    '영어':['레벨별 단어장 + 매일 15개 누적 체크','문장 5형식 기반 독해 훈련','듣기 받아쓰기·섀도잉 주 2회','라이팅 첨삭 리포트 월 2회'],
    '수학':['연산 속도 주간 체크','개념노트 + 예제 풀이 병행','오답 2주·4주 재출제 시스템','서술형 풀이 논리 채점'],
  ...
  const plan = pickN(subjPlan[subject] || subjPlan['국어'], 4, rng);
  const tips = {
  ...
  const fitPool = [
    ...
    ['⚖️','과목을 한 곳에서 해결하고 싶은 학생',subject+' 외 다른 과목도 함께 관리돼 학습 동선이 한 번에 정리됩니다.'],
    ['📅','스케줄 조정이 까다로운 학생',region+' 지역 학교 시간표에 맞춰 수업 시간을 유연하게 조정해 드립니다.'],
    ['💬','상담이 편한 분위기를 찾는 학부모','원장이 직접 상담하고 학생 상태를 구체적으로 공유해 '+region+' 학부모님들 만족도가 높습니다.'],
    ['🎯',subject+' 점수를 단기간에 올리고 싶은 학생','단기 집중 프로그램과 '+region+' 인근 학교 시험 연동으로 최단 경로 성적 향상을 노립니다.'],
  ];
  const fits = pickN(fitPool, 4, rng);
  const otherSubs = (c.s||[]).filter(s => s !== subject).slice(0, 4);
  const faqs = [
    ['Q. '+region+'에서 이곳으로 오는 길이 궁금해요.',c.l ? c.l.substring(0, 150) : (c.a + ' 근처에 위치합니다. 자세한 위치 안내는 상담 시 지도와 함께 전달드립니다.')],
    ['Q. '+region+' 다른 학원과 어떻게 다른가요?','단체 강의가 아닌 1:1 맞춤 코칭을 제공하며, '+region+' 인근 학교 시험 일정과 연동해 커리큘럼을 조정합니다.'],
    ['Q. '+subject+' 외에 다른 과목도 함께 들을 수 있나요?',otherSubs.length ? (otherSubs.join('·') + ' 수업을 병행할 수 있습니다. 한 지점에서 여러 과목 관리가 가능합니다.') : '현재 '+subject+' 중심으로 운영하고 있으며, 상담 시 다른 과목 안내도 가능합니다.'],
  ];
  ...
  return `<!DOCTYPE html><html lang="ko"><head>
  ${commonHead(title+' | '+fullName+' - 과외안하니', region+' '+subject+' 1:1 학원 안내. '+fullName+'이 제공하는 '+rtLabel+' 학생 맞춤 '+subject+' 커리큘럼.', url)}
  <style>${commonStyles()}.rh{position:relative;padding:52px 24px 44px;color:#fff;overflow:hidden;background:linear-gradient(135deg,#0f172a,#1e1b4b 50%,${col} 200%)}.rh::before{content:'';position:absolute;inset:0;background:url('${heroImg}') center/cover;opacity:.12}.ri{position:relative;max-width:800px;margin:0 auto}.rbc{font-size:13px;color:#94a3b8;margin-bottom:14px}.rbc a{color:inherit;text-decoration:none}.rbc a:hover{color:#fff}.rbd{display:inline-block;background:${bg};color:${col};padding:5px 14px;border-radius:20px;font-size:13px;font-weight:700;margin-bottom:14px}.rh h1{font-size:30px;font-weight:900;line-height:1.3;margin-bottom:10px}.rh h1 em{font-style:normal;color:#a5b4fc}.rhs{font-size:15px;color:#cbd5e1;line-height:1.6}.rm{max-width:800px;margin:0 auto;padding:40px 24px 60px}.rx{margin-bottom:36px}.rt{font-size:22px;font-weight:900;color:#0f172a;margin-bottom:14px;display:flex;align-items:center;gap:8px}.rt::before{content:'';width:4px;height:22px;background:${col};border-radius:2px}.rd{font-size:15px;color:#475569;line-height:1.7;margin-bottom:18px}.rg{display:grid;grid-template-columns:1fr 1fr;gap:12px}.rc{background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:16px;transition:transform .15s}.rc:hover{transform:translateY(-2px);border-color:${col}}.rci{font-size:22px;margin-bottom:6px}.rct{font-size:14px;font-weight:800;color:#0f172a;margin-bottom:4px}.rcd{font-size:12.5px;color:#64748b;line-height:1.55}.rp{background:${bg};border-left:4px solid ${col};border-radius:0 12px 12px 0;padding:18px 22px}.rpi{padding:10px 0;border-bottom:1px dashed rgba(15,23,42,.1);font-size:14.5px;color:#0f172a;line-height:1.6}.rpi:last-child{border-bottom:none}.rpi strong{color:${col};margin-right:8px}.rti{background:#fff8ed;border:1px dashed #f59e0b;border-radius:12px;padding:18px 22px;font-size:14.5px;color:#78350f;line-height:1.65}.rfi{background:linear-gradient(135deg,#f8fafc,#fff);border:1px solid #e2e8f0;border-radius:12px;padding:14px 16px;margin-bottom:10px}.rfe{font-size:18px;margin-right:8px}.rft{font-size:14.5px;font-weight:700;color:#0f172a;display:inline}.rfd{font-size:13px;color:#64748b;line-height:1.55;margin-top:4px}.rfa{border-bottom:1px solid #e2e8f0;padding:14px 0}.rfq{font-size:14.5px;font-weight:700;color:#0f172a;margin-bottom:6px}.rfv{font-size:13.5px;color:#475569;line-height:1.6}.rct2{background:linear-gradient(135deg,#0f172a,#312e81);color:#fff;padding:32px 24px;border-radius:16px;text-align:center;margin-top:36px}.rct2 h3{font-size:20px;font-weight:900;margin-bottom:8px}.rct2 p{font-size:14px;color:#a5b4fc;margin-bottom:18px}.rcb{display:flex;gap:10px;justify-content:center;flex-wrap:wrap}.rcbt{padding:12px 22px;border-radius:10px;font-size:14px;font-weight:700;text-decoration:none}.rcp{background:#fff;color:#312e81}.rcf{background:transparent;color:#fff;border:1.5px solid rgba(255,255,255,.3)}@media(max-width:640px){.rg{grid-template-columns:1fr}.rh h1{font-size:24px}}</style></head>
  <body>${navHTML('region')}
  <div class="rh"><div class="ri">
    <div class="rbc"><a href="/">홈</a> &gt; <a href="/학원/전국지점">전국 지점</a> &gt; <a href="/학원/${encodeURIComponent(c.n)}">${c.n}</a> &gt; ${title}</div>
    <div class="rbd">${rtLabel} · ${subject}</div>
    <h1><em>${region}</em> ${subject}학원</h1>
    <p class="rhs">${fullName}이 ${region} 학생들을 위해 운영하는 ${subject} 1:1 맞춤 학원 가이드</p>
  </div></div>
  <div class="rm">
    <div class="rx">
      <div class="rt">${title}을 찾고 있다면</div>
      <p class="rd">${region} 인근에서 ${subject} 학원을 찾는 학생과 학부모를 위해 ${fullName}이 운영하는 ${subject} 1:1 맞춤 프로그램을 안내드립니다. ${rtLabel} 학생들의 학습 특성과 통학 동선을 함께 고려해 설계된 커리큘럼으로 ${subject}의 모든 핵심 영역을 빠짐없이 다룹니다.</p>
    </div>
    <div class="rx">
      <div class="rt">${region} ${subject}학원으로 이곳을 추천하는 이유</div>
      <div class="rg">${reasons.map(r=>`<div class="rc"><div class="rci">${r[0]}</div><div class="rct">${r[1]}</div><div class="rcd">${r[2]}</div></div>`).join('')}</div>
    </div>
    <div class="rx">
      <div class="rt">${subject} 학습 플랜</div>
      <p class="rd">${region} 학생 수준에 맞춰 ${subject}의 핵심 역량을 단계별로 채웁니다. 매 수업마다 이전 학습 진도를 복기한 뒤 신규 개념으로 진행해 공백 없는 누적 학습을 유지합니다.</p>
      <div class="rp">${plan.map((p,i)=>`<div class="rpi"><strong>플랜 ${i+1}.</strong>${p}</div>`).join('')}</div>
    </div>
    <div class="rx">
      <div class="rt">${subject} 공부할 때 꼭 기억할 한 가지</div>
      <div class="rti">💡 ${tips}</div>
    </div>
    <div class="rx">
      <div class="rt">이런 학부모·학생에게 적합합니다</div>
      ${fits.map(f=>`<div class="rfi"><span class="rfe">${f[0]}</span><span class="rft">${f[1]}</span><div class="rfd">${f[2]}</div></div>`).join('')}
    </div>
    <div class="rx">
      <div class="rt">자주 묻는 질문</div>
      ${faqs.map(f=>`<div class="rfa"><div class="rfq">${f[0]}</div><div class="rfv">${f[1]}</div></div>`).join('')}
    </div>
    <div class="rct2">
      <h3>${title} 무료 상담</h3>
      <p>${fullName} · 첫 수업 무료 · 1:1 맞춤 진단</p>
      <div class="rcb">
        <a href="tel:010-6850-1420" class="rcbt rcp">010-6850-1420</a>
        <a href="https://naver.me/GYD2Ki40" target="_blank" class="rcbt rcf">무료 상담 신청</a>
      </div>
    </div>
  </div>
  ${footerHTML()}
...
export default {
  async fetch(request, env) {
    ...
    if (regionSubjMatch) {
      const centerName = regionSubjMatch[1];
